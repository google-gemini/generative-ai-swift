...
public struct CountTokensRequest {
  ...
  public init(model: String, contents: [ModelContent], options: RequestOptions) {
    self.model = model
...
extension CountTokensRequest: GenerativeAIRequest {
  public typealias Response = CountTokensResponse

  public var url: URL {
    URL(string: "\(options.baseURL)/\(options.apiVersion)/\(model):countTokens")!
  }
}
